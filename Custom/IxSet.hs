...
-- | Returns the subset of an index whose key when run in the provided
-- function with the provided key returns True. The set must be indexed 
-- over key type, doing otherwise results in runtime error. The provided
-- key will be the first argument to the provided function.
...
filterByKey func ixset@(IxSet indexes) = collect indexes
    where
    collect [] = error $ "IxSet: there is no index " ++ showTypeOf func
    collect (Ix index _:is) = maybe (collect is) f $ cast index
        where
        f index'' = insertList (Set.toList $ Map.foldrWithKey filterFunc Set.empty index'') empty
            where
            filterFunc k a b = if (func k) then (Set.union a b) else b
